...
class QQUniModel(nn.Module):
    def __init__(self, cfg, bert_cfg_dict, model_path, task=['tag', 'mlm', 'mfm'], init_from_pretrain=True):
        ...
        self.newemb_att = torch.nn.Sequential(
            nn.Linear(uni_bert_cfg.hidden_size, uni_bert_cfg.hidden_size),
            nn.Tanh(),
            nn.Linear(uni_bert_cfg.hidden_size, 1),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, target=None, task=None):
        ...
        # Att emb
        weights = self.newemb_att(features)
        embedding = torch.sum(weights * features, dim=1)
        embedding = self.newfc_hidden(embedding)
        ...
        if 'tag' in sample_task:
            ...
            if target is not None and target[target!=-100].size()[0]!=0:
                tagloss = nn.BCEWithLogitsLoss(reduction="mean")(pred[target!=-100].view(-1), target[target!=-100].view(-1))
                loss += tagloss * 1250 / 3
