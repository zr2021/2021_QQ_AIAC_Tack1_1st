...
def train(model, model_path, 
    ...
    for epoch in range(num_epochs):
        for train_loader, task in train_loader_l:
            for batch_num, item in enumerate(train_loader):
                model.train()
                optimizer.zero_grad()
                pred, emb, loss = get_pred_and_loss(model, item, task)
                loss.backward()
...
for fold in range(NUM_FOLDS):
    ...
    train_dataset_list2 = [f"{DATA_PATH}/test_{ix}/test_{ix}.tfrecords" for ix in ['a', 'b']]
    ...
    train_dataset_list3 = [f"{DATA_PATH}/pairwise/pairwise.tfrecords"]
    train_dataset3 = QQDataset(train_dataset_list3, trans, desc=DESC)
    train_loader3 = DataLoader(train_dataset3, batch_size=BATCH_SIZE, drop_last=True, shuffle=True, num_workers=4)
    ...
    total_steps = NUM_EPOCHS * (len(train_dataset) + len(train_dataset2) + len(train_dataset3)) // BATCH_SIZE
    ...
    train_loader_l = [(train_loader, ['mlm', 'tag', 'mfm']), (train_loader3, ['mlm', 'tag', 'mfm']), (train_loader2, ['mlm', 'mfm'])]
    val_loss = train(model, model_path, train_loader_l, val_loader, optimizer, 
