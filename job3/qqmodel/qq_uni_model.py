...
class QQUniModel(nn.Module):
    def __init__(self, cfg, bert_cfg_dict, model_path, task=['tag', 'mlm', 'mvm', 'itm'], init_from_pretrain=True):
        super().__init__()
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, target=None, task=None):
        ...
        if 'tag' in sample_task:
            pred = self.newfc_tag(torch.relu(embedding))
            if target is not None:
                tagloss = nn.BCEWithLogitsLoss(reduction="mean")(pred.view(-1), target.view(-1)) / 3
                loss += tagloss * 1250
